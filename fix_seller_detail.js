const fs = require('fs');
let code = fs.readFileSync('src/components/SellerOrderDetail.tsx', 'utf-8');
...
code = code.replace(/className="font-semibold break-all"/g, 'className="font-semibold truncate block max-w-full" title={order.buyerEmail || "-"}');

fs.writeFileSync('src/components/SellerOrderDetail.tsx', code);
